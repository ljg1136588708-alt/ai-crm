...
function Button({ children, variant, size, className, href, ...props }: any) {
  ...
  const cls = `${base} ${sizes[size || 'sm']} ${variants[variant || 'default']} ${className || ''}`;
    ...
    <main className="min-h-screen">
      <nav className="flex items-center justify-between px-6 py-4 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          <Link href={l('/pricing')} className="text-sm text-zinc-600 hover:text-zinc-900">{t('common.pricing')}</Link>
          <Link href={l('/sign-in')}><Button variant="outline" size="sm">{t('common.signIn')}</Button></Link>
          <Link href={l('/sign-up')}><Button size="sm">{t('common.signUp')}</Button></Link>
        </div>
      </nav>
      ...
      <section className="max-w-4xl mx-auto text-center pt-32 pb-20 px-6">
        ...
        <div className="flex gap-4 justify-center">
          <Link href={l('/sign-up')}>
            <Button size="lg" className="text-base px-8 bg-violet-600 hover:bg-violet-700">{t('landing.tryFree')}</Button>
          </Link>
          <Link href={l('/pricing')}>
            <Button size="lg" variant="outline" className="text-base px-8">{t('landing.viewPricing')}</Button>
          </Link>
        </div>
      </section>
      ...
      <section className="py-24 px-6 text-center">
        ...
        <Link href={l('/sign-up')}><Button size="lg" className="text-base px-10">{t('landing.tryFree')}</Button></Link>
      </section>
